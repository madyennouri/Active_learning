...
config = load_config('config_v3.json')
...
while len(labeled_indices) < max_labels:
    ...
    for epoch in range(num_epochs_per_iter):
        ...
        print(
            f'Iteration [{iter_num}] | Epoch [{epoch_num}/{num_epochs_per_iter}], Loss: {np.mean(epoch_losses):.4f}, Loss prediction Loss: {np.mean(epoch_losses_pred_loss):.4f}')
    iter_num += 1
