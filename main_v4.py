...
config = load_config('config_v4.json')
...
while len(labeled_indices) < max_labels:
    ...
    new_label = compute_result(inputs[batch_indices, 0], inputs[batch_indices, 1])
...
iter_vec = np.arange(1, len(losses) + 1, 1)
print(len(labeled_indices))
fig, ax = plt.subplots(figsize=(5, 5))
ax.errorbar(iter_vec, losses, fmt='-k', label='Main Loss', marker='o', capsize=3)
ax.errorbar(iter_vec, losses_pred_loss, fmt='-b', label='Loss Prediction Loss', marker='v', capsize=3)
